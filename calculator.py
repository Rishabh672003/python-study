import math
print("THis is a program to solve a equation in the form ax^3 + bx^2 + cx + d = 0")
...
if a == 0:
    if b == 0:
        ...
        if math.pow(c, 2) - 4 * b * d > 0:
            ...
            print("Roots are", (-c + math.sqrt(c ** 2 - 4 * b * d)) / (2 * b), "and",
                  (-c - math.sqrt(c ** 2 - 4 * b * d)) / (2 * b))
        elif math.pow(c, 2) - 4 * b * d < 0:
            print("Roots are Complex and Imaginary ")
        elif math.pow(c, 2) - 4 * b * d == 0:
            print("Roots are Real and Equal")
            print("both Roots are ", (-c - math.sqrt(c ** 2 - 4 * b * d)) / (2 * b))
else:
